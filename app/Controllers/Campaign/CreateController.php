...
<div class="col-12 p-1 d-flex post-btn-box flex-wrap align-items-center border-top border-bottom">
        ...
        if ($_POST['action'] == 'account_list') {
            ...
            if ($data_count > 0) {
                ...
                foreach ($fb_account_data as $account_key => $account_value) {
                    ...
                    $master_id = $_SESSION['master'];
                    $get_page_data2 = 'SELECT * FROM ' . $asset_table_name . ' WHERE platform_id = "' . $platform_id . '" AND asset_type="pages" AND master_id=' . $master_id . ';';
                    $get_page_data1 = $this->db->query($get_page_data2);
                    $get_page_data = $get_page_data1->getResultArray();
                    // pre($get_page_data);
                    ...
                    foreach ($fb_page_list['data'] as $key => $value) {
                        if (in_array($value['id'], $perasset_data) || (isset($_SESSION['admin']) && $_SESSION['admin'] == 1)) {
                            ...
                            $fb_chat_list_html .= '<div class="col-12 account-nav account-box linked-page" data-img="' . $value['asset_img'] . '" page-name="' . $value['name'] . '" data-pagee_id="' . $value['asset_id'] . '" data-platform="messenger" data-page_access_token="' . $value['access_token'] . '" data-page_name="' . $value['name'] . '">
	...
	';
        ...
        if (isset($_POST['action'])) {
            if ($_POST['action'] == 'img_load') {
                $page_access_token = $_POST['access_tocken'];
                $page_id = $_POST['data_pageasset_id'];
                $data_img1 = $_POST['data_img'];
                $page_namee = $_POST['page_name'];
    ...
    <img class="rounded-circle" src="' . $data_img1 . '" alt="#" style="width:40px;height:40px;">
    </div>
    <div class="col">
    <div class="col-12 d-flex flex-wrap justify-content-between">
    ...
    <div class="col-12 d-flex justify-content-start">
    <button class="bg-transparent text-start ps-1 col-12 border-0 edit_post_facebook" data-edit_id="' . $value['id'] . '" data-page_id="' . $page_id . '" data-access_token="' . $accesss_tocken_comment . '" data-bs-toggle="modal" data-bs-target="#staticBackdrop"><i class="fa-solid fa-trash-can me-2"></i>Edit</button>
    </div>
    </div>
    </ul>
    </div>
